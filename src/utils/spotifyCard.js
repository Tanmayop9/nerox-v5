...
export async function generateSpotifyCard(track, requester) {
    ...
    const titleText = track.title?.length > 25 
    ...
    ctx.fillStyle = TEXT_MEDIUM;
    ctx.font = '22px Arial';
    const authorText = track.author?.length > 30 
    ...
    const progressWidth = Math.min(barWidth, barWidth * 0.10); 
    ctx.fillStyle = ACCENT_COLOR;
    ctx.beginPath();
    ...
    ctx.fillStyle = ACCENT_COLOR;
    ctx.font = 'bold 18px Arial';
    const requesterName = requester?.displayName?.length > 20 
